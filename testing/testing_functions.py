...
# Task 2 using multithreading to create histogram data
def get_route(station_outer, station_inner, station_data_graph, vertices):
    ...
    return station_count - 2


def get_graph_csv_task_2_multithread(station_data_graph, vertices):
    top_column = ['STARTS ->']
    output = []
    full_run_time_start = time.time()
    pool = mp.Pool(processes=8)  # Create a pool of 8 processes
    for station_outer in vertices:  # Use station_outer as start
        ...
        results = [pool.apply_async(get_route, args=(station_outer, station_inner, station_data_graph, vertices)) for station_inner in vertices]
        next_column.extend([p.get() for p in results])
        ...
        end_time = time.time()
        print('Processing done for: ' + station_outer + ' | ' + str(len(next_column)) + ' data points | time taken: ' + str(round(end_time - start_time, 3)) + 's')
...
# Run this to generate CSVs with multithreading for histogram graphs
if __name__ == '__main__':
    # Get graph and vertices
    underground_graph, vertices = data_processing.get_data()

    # FOR HISTOGRAM CREATION:
    get_graph_csv_task_2_multithread(underground_graph, vertices)
